...
app.get('/users/:id', async (req, res) => {
    try {
        ...
        const results = await conn.query(
            'SELECT customer.cus_id, customer.cus_mail, ROUND(SUM(history.his_profit)*0.1) AS payment ' +
            'FROM customer ' +
            'INNER JOIN history ON customer.cus_port = history.cus_port ' +
            'WHERE customer.id = ? ' +
            'GROUP BY customer.id, customer.cus_mail',
            [id]
        );
...
//     const sql = "insert into payment SET image = ?";
//     await conn.query(sql, [image], (err, result) => {
...
//         })
//     }catch (error) {
//         console.error('error message', error) ///ฝั่งเราเห็น
//         res.status(500).json({
//             message: 'somthing wrong',
//             //errorMessage: error.message //user จะเห็นerror เลยไม่ใช้ไป consoleข้างบน
//         })
